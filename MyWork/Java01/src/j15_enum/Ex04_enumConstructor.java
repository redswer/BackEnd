...
public class Ex04_enumConstructor {
	...
	public static void main(String[] args) {
		...
		switch (f) {
		case APPLE:
			System.out.println("* APPLE, color => " + Fruit.APPLE.color); // 변수접근
			break;
		case BANANA:
			System.out.println("* BANANA, getColor=> " + Fruit.BANANA.getColor()); // 메서드접근
			break;
		case PEACH:
			System.out.println("* APPLE, color => " + Fruit.PEACH.color);
			break;
		} // switch
		...
		// if (Fruit.APPLE == Company.APPLE ) System.out.println(" ** 컴파일 오류 발생 => 그러므로
		// 안전 **");
		// => 컴파일 오류 , 비교구문 불가능 : 그러므로 안전
		Company c = Company.APPLE;
		switch (c) {
		case APPLE:
			System.out.println(c + " => 연봉 5천 ");
			break;
		case NAVER:
			System.out.println(c + " => 연봉 6천 ");
			break;
		case ORACLE:
			System.out.println(c + " => 연봉 7천 ");
			break;
		}
