...
public class DBConnection {
	...
	public static Connection getConnection() {
		try {
			...
			// => new 와 다르게 생성자를 실행하지 않고 인스턴스 생성
			...
			// => allowPublicKeyRetrieval=true : local DB open 하지 않아도(mySQL 실행되어 있지 않아도)
			// connection 허용
			// => localhost = @127.0.0.1 (IP 주소)

			Connection cn = DriverManager.getConnection(url, "root", "mysql");
			// => .getConnection(String url, String user, String password) 에서
			// => mySQL의 기본 user 는 root
