...
public class Ex01_Flow01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 1. Forward
		// => 웹 브라우저의 주소창이 바뀌지 않으며, 요청명과 다른 결과가 실행됨
		// -> 요청명은 flow01 이지만 실행 결과는 helloS

		response.setContentType("text/html; charset=UTF-8");
		PrintWriter out = response.getWriter();

		out.print("<h2>** Forward Test (servlet -> servlet) **</h2>");
		// => 요청은 flow01 이지만 helloS 가 실행되므로 출력구문 실행되지 않음
		// -> console 로 확인

		System.out.println("** Forward Test 1 (servlet -> servlet) **");
//		------------------------------
		// Forward 1) servlet -> servlet (to helloS)
		String uri = "helloS";

		// Forward 2) Servlet -> JSP (to form04_select)
...
//		request.getRequestDispatcher(uri).forward(request, response);
		// => 주소창은 http://localhost:8080/Web01/flow01
...
//		======================================================
		...
		// ** 위의 forward 요청(request.getRequestDispatcher()) 막고 실행

		// Redirect 1) servlet -> servlet (to helloS)
		uri = "helloS";

		// Redirect 2) servlet -> JSP (to form04_select)
		uri = "servletTestForm/form04_Select.jsp";

//		------------------------------
		response.sendRedirect(uri);
		// => 주소가 .../helloS , .../form04_select.jsp 로 변경
	}
