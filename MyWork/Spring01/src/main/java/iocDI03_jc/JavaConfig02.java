...
public class JavaConfig02 {
	...
	@Bean
	// => 컨테이너에 의해 메서드가 실행되고, 만들어진 인스턴스가 다시 컨테이너로 return 됨
	public TV sstv() {
		return new SsTVs();
	}

	// 2) IOC/DI: 생성자 주입
	// => Speaker 생성 후 사용 (하나의 컨테이너에서만 사용하면 생성과 동시에 사용해도 무관)
	...
	@Bean
	public TV lgtv() {
		return new LgTVs(sp(), "pink", 9988000);
	}
//	@Bean
//	public TV lgtv() {
//		return new LgTVs(new Speaker(), "pink", 9988000);
//		// => Speaker 생성과 동시에 사용
//	}
